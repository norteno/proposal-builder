...
import { Proposal, TimelineUnit } from "@/lib/types";
...
function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: { label: string; value: string }[] }) {
  return (
    <label className="block space-y-2">
      <span className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-500">{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-neutral-900">
        {options.map((option) => (
...
export default function EditorPanel({ proposal, setProposal, activePanel, duplicateProposal, saveNow, deleteCurrent }: { proposal: Proposal; setProposal: React.Dispatch<React.SetStateAction<Proposal>>; activePanel: Panel; duplicateProposal: () => void; saveNow: () => void; deleteCurrent: () => void; }) {
  ...
  const timelineUnitLabel = proposal.timelineUnit === "weeks" ? "Week" : "Month";
  ...
  return (
    <section className="w-full border-r border-neutral-200 bg-neutral-50 p-4 lg:w-[420px] lg:shrink-0 lg:overflow-y-auto">
      ...
      {activePanel === "content" && <Card><CardContent className="space-y-5">
        ...
        <Field label="Hero Created Date" value={proposal.heroCreatedDate} onChange={(value) => update("heroCreatedDate", value)} />
