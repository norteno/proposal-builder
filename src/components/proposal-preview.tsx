  ...
  return (
    <div role="main" className={clean ? "min-h-screen bg-neutral-200" : "flex-1 overflow-y-auto bg-neutral-200"}>
      <div className="proposal-preview-shell w-full overflow-hidden" style={themeVars}>
        ...
        <section className="bg-[var(--secondary)]" style={{ color: proposal.sectionTextColors.team }}><SectionInner><p className="mb-8 text-xs font-bold uppercase tracking-[0.22em] opacity-100">The proposed team</p><div className="grid grid-cols-2 gap-5 md:grid-cols-3 lg:grid-cols-6">{proposal.team.map((member, index) => <div key={`team-preview-${index}`}><ImageOrPlaceholder src={member.imageUrl} label={member.name} className="aspect-[2/3] w-full rounded-2xl bg-white/90 object-cover grayscale" /><p className="mt-3 text-sm font-bold">{member.name}</p><p className="text-xs" style={{ color: proposal.sectionBodyColors.team }}>{member.role}</p></div>)}</div></SectionInner></section>
