      ...
      <div className="proposal-preview-shell w-full overflow-hidden" style={themeVars}>
        ...
        <section className="bg-[var(--cream)]" style={{ color: proposal.sectionTextColors.about }}>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[520px_1fr]">
            ...
            <div><p className="text-xs uppercase tracking-[0.22em] opacity-50">About the studio</p><h2 className="mt-4 max-w-lg font-display text-4xl font-black leading-none tracking-[-0.02em] sm:text-5xl">{proposal.aboutTitle}</h2><p className="mt-8 max-w-3xl text-lg leading-8" style={{ color: proposal.sectionBodyColors.about }}>{proposal.aboutBody}</p></div>
          </div>
        ...
        <section className="bg-[var(--cream)]" style={{ color: proposal.sectionTextColors.deliverables }}><SectionInner><p className="text-xs uppercase tracking-[0.24em] opacity-50">Scope</p><h2 className="mt-2 font-display text-4xl font-black tracking-[-0.02em]">Web Design</h2><div className="mt-12 grid gap-10 md:grid-cols-2">{proposal.deliverables.map((item, index) => <article key={`deliverable-preview-${index}`} className="rounded-[1.75rem] border border-current/15 bg-[var(--cream)] p-6"><p className="text-xs font-bold uppercase tracking-[0.2em] opacity-50">{item.phase}</p><h3 className="mt-3 font-display text-2xl font-black tracking-[-0.01em]">{item.title}</h3><p className="mt-4 text-sm leading-6" style={{ color: proposal.sectionBodyColors.deliverables }}>{item.description}</p><div className="mt-6 rounded-2xl border border-current/20 p-4"><p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] opacity-50">Includes</p><ul className="space-y-2 text-sm" style={{ color: proposal.sectionBodyColors.deliverables }}>{item.items.map((included, itemIndex) => <li key={`included-${index}-${itemIndex}`} className="flex gap-2"><span>•</span><span>{included}</span></li>)}</ul></div></article>)}</div></SectionInner></section>
