...
type ProposalVars = CSSProperties & {
  "--primary": string;
  "--secondary": string;
  "--cream": string;
  "--accent": string;
...
export function ProposalPreview({ proposal }: { proposal: Proposal }) {
  const themeVars = useMemo<ProposalVars>(
    () => ({
      ...
      fontFamily: proposal.theme.bodyFont,
    }),
    [proposal.theme]
  );

  const titleWords = proposal.title.split(" ");
  const lastWord = titleWords.at(-1) ?? "Proposal";
  const firstWords = titleWords.slice(0, -1).join(" ");

  return (
    <main className="flex-1 overflow-y-auto bg-neutral-200 p-3 sm:p-5">
      <div className="mx-auto max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-2xl" style={themeVars}>
        <section className="relative min-h-[680px] bg-[var(--primary)] px-8 py-10 text-[var(--proposal-text)] sm:px-16 sm:py-14">
          ...
          >
            ...
            <h1 className="mt-6 text-5xl font-black leading-[0.92] tracking-[-0.06em] sm:text-7xl" style={{ fontFamily: proposal.theme.headingFont }}>
              {firstWords} <em className="font-serif text-[var(--accent)]">{lastWord}</em>
            </h1>
        ...
        <section className="bg-[var(--secondary)] px-8 py-20 text-white sm:px-24">
          <div className="mx-auto max-w-2xl">
            ...
            <div className="mt-10 flex gap-8 text-sm text-white/60">
              <div><div className="h-10 w-10 rounded-full bg-white/20" /><p className="mt-3 text-white">Amanda Spain</p><p>Creative Partner</p></div>
              <div><div className="h-10 w-10 rounded-full bg-white/20" /><p className="mt-3 text-white">TGS Team</p><p>Design & Development</p></div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-[var(--cream)] px-8 py-20 text-[var(--dark-text)] sm:px-20">
          <div className="grid items-center gap-10 lg:grid-cols-[280px_1fr]">
            <div className="flex gap-3">
              {[0, 1, 2].map((item) => <div key={item} className="h-32 w-24 rounded-t-full bg-neutral-800/80 shadow-xl" />)}
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-neutral-500">About the studio</p>
              <h2 className="mt-4 max-w-lg text-4xl font-black leading-none tracking-[-0.05em] sm:text-5xl" style={{ fontFamily: proposal.theme.headingFont }}>{proposal.aboutTitle}</h2>
          ...
          <div className="mt-16 flex flex-wrap items-center justify-between gap-6 border-t border-neutral-900/10 pt-8">
            {proposal.clientLogos.map((logo) => <span key={logo} className="text-sm font-black uppercase tracking-[-0.04em] text-neutral-900/70">{logo}</span>)}
          </div>
        </section>
        ...
        <section className="bg-[var(--primary)] px-8 py-20 text-white sm:px-20">
          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
            <div>
              ...
              <h2 className="mt-4 max-w-xl text-4xl font-black leading-none tracking-[-0.05em] sm:text-5xl" style={{ fontFamily: proposal.theme.headingFont }}>{proposal.proofTitle}</h2>
              <p className="mt-8 max-w-2xl text-lg leading-8 text-white/70">{proposal.proofBody}</p>
            </div>
            <div className="aspect-[4/3] rounded-3xl bg-white/15 p-4 shadow-2xl"><div className="h-full rounded-2xl bg-gradient-to-br from-white/80 to-white/20" /></div>
          </div>
        </section>

        <section className="bg-[var(--secondary)] px-8 py-20 text-white sm:px-20">
          <p className="mb-8 text-xs font-bold uppercase tracking-[0.22em] text-white/50">The proposed bean team</p>
          <div className="grid grid-cols-2 gap-5 md:grid-cols-3 lg:grid-cols-6">
            {proposal.team.map((member, index) => (
              <div key={`${member.name}-${index}`}>
                {member.imageUrl ? (
                  <img src={member.imageUrl} alt={member.name} className="aspect-square rounded-2xl object-cover grayscale" />
                ) : (
        ...
        <section className="bg-[var(--cream)] px-8 py-20 text-[var(--dark-text)] sm:px-20">
          ...
          <h2 className="mt-2 text-4xl font-black uppercase tracking-[-0.06em]" style={{ fontFamily: proposal.theme.headingFont }}>Web Design</h2>
          <div className="mt-12 grid gap-10 md:grid-cols-2">
            {proposal.deliverables.map((item) => (
              <article key={`${item.phase}-${item.title}`} className="rounded-[1.75rem] border border-neutral-900/15 bg-[var(--cream)] p-6">
                ...
                <h3 className="mt-3 text-2xl font-black tracking-[-0.05em]" style={{ fontFamily: proposal.theme.headingFont }}>{item.title}</h3>
                ...
                <div className="mt-6 rounded-2xl border border-neutral-900/20 p-4">
                  ...
                  <ul className="space-y-2 text-sm text-neutral-800">
                    {item.items.map((included) => <li key={included} className="flex gap-2"><span>•</span><span>{included}</span></li>)}
                  </ul>
                </div>
              </article>
            ))}
        ...
        <section className="flex min-h-[320px] items-end justify-between bg-[var(--primary)] px-8 py-12 text-white sm:px-20">
          ...
          <h2 className="text-4xl font-black uppercase tracking-[-0.06em] text-white/20">Handoff</h2>
