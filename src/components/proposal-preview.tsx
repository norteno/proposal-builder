  ...
  return (
    <div role="main" className={clean ? "min-h-screen bg-neutral-200" : "flex-1 overflow-y-auto bg-neutral-200"}>
      <div className="proposal-preview-shell w-full overflow-hidden" style={themeVars}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mt-44 max-w-[51rem]">
            ...
            <div className="mt-6 grid gap-6 text-sm sm:grid-cols-[280px_220px]">
              ...
              <div className="border-l border-[#061D00]/20 pl-10">
                ...
                <p className="mt-2">The Graphic Standard</p>
              </div>
            </div>
          </motion.div>
          </SectionInner>
        ...
        <section className="bg-[var(--secondary)]" style={{ color: proposal.sectionTextColors.team }}><SectionInner><p className="mb-8 text-xs font-bold uppercase tracking-[0.22em] opacity-100">The proposed team</p><div className="grid grid-cols-2 gap-5 md:grid-cols-3">{proposal.team.map((member, index) => <div key={`team-preview-${index}`}><ImageOrPlaceholder src={member.imageUrl} label={member.name} className="aspect-square w-full rounded-2xl bg-white/90 object-cover grayscale" /><p className="mt-3 text-sm font-bold">{member.name}</p><p className="text-xs" style={{ color: proposal.sectionBodyColors.team }}>{member.role}</p></div>)}</div></SectionInner></section>
        ...
        <section className="bg-[var(--primary)]" style={{ color: proposal.sectionTextColors.handoff }}><SectionInner className="min-h-[120px] py-12" contentClassName="flex min-h-[120px] items-end justify-center text-center"><h2 className="mt-6 max-w-xl text-lg leading-8">Thank you for considering TGS. Contact: Shane Bzdok
shane@thegraphicstandard.com
512.761.0367</h2></SectionInner></section>
      </div>
    </div>
  );
}
