...
function TimelineSection({ proposal }: { proposal: Proposal }) {
  ...
  return (
    <section className="bg-[#f8f7f4]" style={{ color: proposal.sectionTextColors.timeline }}>
      ...
      <h2 className="mt-5 font-display text-5xl font-black tracking-[-0.03em] sm:text-6xl">Estimated Timeline</h2>
...
function PricingSection({ proposal }: { proposal: Proposal }) {
  return (
    <section className="bg-[var(--primary)]" style={{ color: proposal.sectionTextColors.pricing }}>
      ...
      <h2 className="mt-5 font-display text-5xl font-black tracking-[-0.03em] sm:text-6xl">{proposal.pricing.title}</h2>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {proposal.pricing.items.map((item, index) => (
          <article key={`pricing-card-${index}`} className="rounded-3xl border border-white/10 p-8" style={{ backgroundColor: proposal.pricing.cardBackgroundColor }}>
            ...
            <h3 className="mt-5 font-display text-2xl font-black tracking-[-0.02em]">{item.title}</h3>
      ...
      <div className="proposal-preview-shell w-full overflow-hidden" style={themeVars}>
        <section className="relative min-h-[680px] bg-[var(--primary)]" style={{ color: proposal.sectionTextColors.hero }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mt-44 max-w-[51rem]">
            ...
            <h1 className="mt-6 font-display text-5xl font-black leading-[0.92] tracking-[-0.02em] sm:text-7xl">{titleWords.join(" ")} <em className="font-serif text-[var(--accent)]">{lastWord}</em></h1>
        ...
        <section className="bg-[var(--cream)]" style={{ color: proposal.sectionTextColors.about }}>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[520px_1fr]">
            ...
            <div><p className="text-xs uppercase tracking-[0.22em] opacity-50">About the studio</p><h2 className="mt-4 max-w-lg font-display text-4xl font-black leading-none tracking-[-0.03em] sm:text-5xl">{proposal.aboutTitle}</h2><p className="mt-8 max-w-3xl text-lg leading-8" style={{ color: proposal.sectionBodyColors.about }}>{proposal.aboutBody}</p></div>
          </div>
        ...
        <section className="bg-[var(--primary)]" style={{ color: proposal.sectionTextColors.experience }}>
          <SectionInner>
          <div className="grid gap-10 lg:grid-cols-[1fr_520px]"><div><p className="text-xs uppercase tracking-[0.22em] opacity-40">Experience</p><h2 className="mt-4 max-w-xl font-display text-4xl font-black leading-none tracking-[-0.02em] sm:text-5xl">{proposal.proofTitle}</h2><p className="mt-8 max-w-2xl text-lg leading-8" style={{ color: proposal.sectionBodyColors.experience }}>{proposal.proofBody}</p></div><div className="h-[400px] overflow-hidden rounded-3xl">{proposal.experienceImageUrl ? <ImageOrPlaceholder src={proposal.experienceImageUrl} label="Experience" className="h-full w-full rounded-3xl bg-white/20 object-cover" /> : <div className="h-full rounded-3xl bg-gradient-to-br from-white/80 to-white/20" />}</div></div>
          </SectionInner>
        </section>

        <section className="bg-[var(--secondary)]" style={{ color: proposal.sectionTextColors.team }}><SectionInner><p className="mb-8 text-xs font-bold uppercase tracking-[0.22em] opacity-50">The proposed team</p><div className="grid grid-cols-2 gap-5 md:grid-cols-3 lg:grid-cols-6">{proposal.team.map((member, index) => <div key={`team-preview-${index}`}><ImageOrPlaceholder src={member.imageUrl} label={member.name} className="aspect-square w-full rounded-2xl bg-white/90 object-cover grayscale" /><p className="mt-3 text-sm font-bold">{member.name}</p><p className="text-xs" style={{ color: proposal.sectionBodyColors.team }}>{member.role}</p></div>)}</div></SectionInner></section>

        <section className="bg-[var(--cream)]" style={{ color: proposal.sectionTextColors.deliverables }}><SectionInner><p className="text-xs uppercase tracking-[0.24em] opacity-50">Scope</p><h2 className="mt-2 font-display text-4xl font-black uppercase tracking-[-0.03em]">Web Design</h2><div className="mt-12 grid gap-10 md:grid-cols-2">{proposal.deliverables.map((item, index) => <article key={`deliverable-preview-${index}`} className="rounded-[1.75rem] border border-current/15 bg-[var(--cream)] p-6"><p className="text-xs font-bold uppercase tracking-[0.2em] opacity-50">{item.phase}</p><h3 className="mt-3 font-display text-2xl font-black tracking-[-0.02em]">{item.title}</h3><p className="mt-4 text-sm leading-6" style={{ color: proposal.sectionBodyColors.deliverables }}>{item.description}</p><div className="mt-6 rounded-2xl border border-current/20 p-4"><p className="mb-3 text-xs font-bold uppercase tracking-[0.18em] opacity-50">Includes</p><ul className="space-y-2 text-sm" style={{ color: proposal.sectionBodyColors.deliverables }}>{item.items.map((included, itemIndex) => <li key={`included-${index}-${itemIndex}`} className="flex gap-2"><span>•</span><span>{included}</span></li>)}</ul></div></article>)}</div></SectionInner></section>
